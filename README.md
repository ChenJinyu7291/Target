...
- The Reviewer adapter was retrained on 2026-08-10 against the current dual-reviewed 120-row SFT set (data baseline commit `6de64c9`). Heldout acceptance (30 rows): adapter `json_valid`/`category_match`/`no_false_complete`/`safe_action`/`fully_correct` all 1.0 vs base `category_match`/`safe_action`/`fully_correct` 0.0; model weights are not tracked in Git. The adapter remains an optional probe-based Reviewer backend: deterministic gates stay authoritative, and a 30-row template-consistent heldout does not establish open-world Reviewer quality.
